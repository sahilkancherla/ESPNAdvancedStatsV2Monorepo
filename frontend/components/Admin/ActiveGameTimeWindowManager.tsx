...
interface ActiveGameTimeWindow {
  id: number;
  start_time: string;
  end_time: string;
  week: number;
...
export const ActiveGameTimeWindowManager = () => {
  const [timeWindows, setTimeWindows] = useState<ActiveGameTimeWindow[]>([]);
  const [isFormOpen, setIsFormOpen] = useState(false);
  const [editingId, setEditingId] = useState<number | null>(null);
  const [selectedYear, setSelectedYear] = useState<string>(new Date().getFullYear().toString());
  const [formData, setFormData] = useState<ActiveGameFormData>({
    date: '',
    startTime: '',
    endTime: '',
    week: '',
  ...
  useEffect(() => {
    const fetchTimeWindows = async (year: string) => {
      try {
        const response = await fetch(`${BACKEND_URL}/admin/getActiveGameTimeWindows?year=${year}`);
        ...
        console.error('Error fetching active game time windows:', error);
        showAlert('Failed to fetch active game time windows', 'error');
      }
    };
  ...
  const saveToDatabase = async (window: ActiveGameFormData) => {
    // Convert local datetime to UTC
    const startDateTime = new Date(`${window.date}T${window.startTime}`);
    const endDateTime = new Date(`${window.date}T${window.endTime}`);
    
    try {
      const response = await fetch(`${BACKEND_URL}/admin/addNewActiveGameTimeWindow`, {
        method: 'POST',
        headers: {
      ...
      if (!response.ok) {
        throw new Error(`Failed to create active game time window: ${response.status}`);
      }

      const result = await response.json();
      return result.data[0];
    } catch (error) {
      console.error('Error creating active game time window:', error);
      throw error;
    }
  };

  const updateInDatabase = async (id: number, window: ActiveGameFormData) => {
    const startDateTime = new Date(`${window.date}T${window.startTime}`);
    const endDateTime = new Date(`${window.date}T${window.endTime}`);
    
    try {
      const response = await fetch(`${BACKEND_URL}/admin/updateActiveGameTimeWindow`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const deleteFromDatabase = async (id: number) => {
    try {
      const response = await fetch(`${BACKEND_URL}/admin/deleteActiveGameTimeWindow`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (!response.ok) {
        throw new Error(`Failed to delete active game time window: ${response.status}`);
      }
  ...
  const handleEdit = (window: ActiveGameTimeWindow) => {
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-3xl font-bold">Active Game Time Window Manager</h1>
...
export default ActiveGameTimeWindowManager;